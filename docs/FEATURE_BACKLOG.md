...
- [ ] **Hero ignores the author's own accent colour** — found Aug 16, 2026 while verifying the above on a live site. `hero-banner.tsx` takes its accent from `getHeroColors(author.siteTheme)` (the *theme's* accent), while every template takes it from the author's resolved `author.accentColor`. On a site where those differ, the hero's "Buy Now" and the section below it render in two different colours on the same screen — visible right now on `apbedford` (coral hero, grey subscribe button). Either the hero should honour the author's accent, or the resolution needs to be one shared decision rather than two. Touches every author's hero, so worth a deliberate call rather than a drive-by fix. *(small–medium, but high visibility)*
...
- [ ] **Contrast-guard `accentColor` when it's used as TEXT** — partially addressed Aug 16, 2026: text *on* the accent (buttons) now measures real WCAG contrast via `readableTextOn` in `src/lib/color-contrast.ts`. The inverse case is still open — Classic and Minimal paint the raw accent as text on white (section eyebrows, "About the Author" labels, series links) with no guard, so a pale accent gives body text failing AA. `contrastRatio` now exists to check it; needs a darkened-shade fallback for text-only contexts. Note one accent can't be fixed by choosing between two candidates at all: a mid indigo `#6366F1` tops out at 4.47:1 on white, just under AA — that class needs shade adjustment, not selection. *(small)*
