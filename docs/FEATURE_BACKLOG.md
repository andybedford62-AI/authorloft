...
- ✅ **Bundle & Course discount codes** — `DiscountCodeBundle`/`DiscountCodeCourse` join tables extend the existing per-book `DiscountCode` restriction pattern (empty list = applies to all of that type). Admin UI at `/admin/discount-codes` gained bundle/course checkbox sections. Fixed a real bug in the same pass: bundle checkout accepted a `discountCode` param and resolved it, but silently discarded the calculation — the charge was always full price with no error shown. Course checkout had zero discount handling before this (net new). Both `bundle_purchase`/`course_purchase` Stripe webhook blocks now increment `usesCount`; new shared `DiscountCodeInput` component wired into book buy widgets, cart, and the bundle/course buy buttons. All plans (not tier-gated). (August 11, 2026)
