...
- [ ] **Bring Classic and Minimal up to Cinematic's bar** — the four templates aren't the same product. Cinematic reads like a current editorial site (personalized copy, tilt covers, scenic hero backdrops); Classic and Minimal read like the platform's 2016-era design (gray-900 headings, `ChevronRight` breadcrumbs everywhere, sections tinted by stacking accent colour at low opacity over white). An author picking Classic for genre fit is silently getting an older design era, not a style choice. Not a rebuild — spacing, one confident type scale, and personalized microcopy, reusing patterns already proven in Cinematic. Type-scale piece done Aug 17, 2026: Classic's section headings were a flat `text-2xl font-bold` with no responsive scaling at any breakpoint, identical for every section regardless of importance — bumped to `text-2xl sm:text-3xl font-extrabold tracking-tight` (the pattern Bold already proved) across all four. Minimal turned out to have an *intentional* hierarchy already (code comments call its newsletter section "deliberately the most restrained of the four") — flattening it to Classic's treatment would have fought that on purpose, so it kept its existing sizes/weights and relative order, just gained the responsive scaling it was missing (Books/Series/About each step up one breakpoint; the newsletter heading, explicitly the quietest by design, untouched). Spacing and personalized microcopy are still open — copy in particular needs real judgment calls per template, not a mechanical pass. *(medium → spacing + microcopy remaining)*
