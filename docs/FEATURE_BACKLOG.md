...
- [ ] **Course lesson PDF export** — "Download this lesson" (or whole course) as a PDF of the written content; video lessons show the source video URL as a reference link rather than an embedded/downloadable video (no legitimate way to let readers download someone else's YouTube/Vimeo-hosted video). No PDF-generation library exists in the codebase yet — needs a spike to pick one (e.g. a headless-Chrome HTML→PDF render, or a lighter pure-JS PDF builder) before scoping further. Consider soon, not urgent. *(medium — new infra)*
- [ ] **Bulk course upload (CSV/Excel template)** — supplement the current one-lesson-at-a-time admin editor with a template-based import: author fills out a spreadsheet (one row per lesson — course title, module title + order, lesson title + order, lesson content, video URL, `isPreview`) and uploads it to bulk-create the course structure, then fine-tunes in the existing editor (formatting, images, attachments) afterward. Don't build the CSV parsing/mapping layer from scratch — `BookImportWizard` (`src/components/admin/book-import-wizard.tsx` + `src/lib/csv-import.ts`) already has the exact shape needed: `papaparse` CSV parsing, a downloadable template file (`/public/templates/authorloft-books-import-template.csv`), auto-detected column presets with a manual column-mapper fallback. A course version would mirror that (`CourseFieldKey`/`COURSE_FIELDS`, a `CourseImportWizard`, a new template CSV) rather than reinvent it. True `.xlsx` (not just CSV) would need a new dependency (e.g. `xlsx`/`exceljs`) — start with CSV like the book importer does, since it already handles Excel-exported CSV fine. *(medium — mostly adapting an existing pattern)*
