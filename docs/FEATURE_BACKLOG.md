...
- [x] **Music lists on author sites** — shipped Aug 24, 2026. Link-only playlists (YouTube/Spotify embed inline; Suno and unknown hosts render as link cards, since Suno sends `frame-ancestors 'none'`). Shares the Course tree via `Course.kind`; `/music` + `/music/[slug]` public, `/admin/music` CRUD, `navShowMusic` toggle, 5/20/unlimited lists and 15/50/unlimited tracks. No uploads or storage by design. *(done)*
- [ ] **Music: uploaded audio** — deferred Aug 24, 2026 as explicitly too expensive for now (storage + bandwidth). If revisited: `AudioPlayer` (`src/components/author-site/audio-player.tsx`) is already a prop-driven multi-track player, and `/api/admin/upload/audio` already accepts mp3/m4a/wav/ogg/flac/aac at 100MB — but `media-src` in the CSP currently allows only self/Supabase/S3, and there is no per-author storage quota anywhere on the platform. Decide the quota before building. *(medium)*
- [ ] **Music: sellable lists** — not built. Checkout, enrol and the discount picker are all guarded to `kind: COURSE` today, so enabling this means deliberately widening those, plus a `MUSIC` OrderItem type. Revenue-critical; scope separately. *(medium)*
...
- [ ] **⭐ Course import that a real prospect would actually use (NEXT SESSION — Andy flagged Aug 21, 2026)** — the strategic gap, not a specific ticket. Today a course creator evaluating AuthorLoft has exactly two paths, and both are bad at the moment that matters most (first 20 minutes, deciding whether to switch):
  1. **Hand-build** every module and lesson in the admin editor — fine for a new course, unusable for migrating an existing 40-lesson curriculum.
  2. **The CSV importer** (`/admin/courses/import`) — real and working, but it assumes the creator will hand-author a one-row-per-lesson spreadsheet. Nobody's existing course lives in that shape, so "import" still means a manual cut-and-paste job, just into Excel instead of our editor.

  **The actual problem to solve:** get a creator's course *in the form they already have it* into AuthorLoft with minimal manual retyping. Their content realistically lives as one of: a Teachable/Thinkific/Kajabi export, a folder of PDFs/DOCX/slides, a YouTube playlist, a Notion/Google Doc outline, or a zip of video files.

  **Directions worth weighing (decide before building — do not default to more CSV):**
  - *Platform export presets* — highest fidelity, but blocked on real sample exports (see the separate line below). Ask a prospect for one; a single real Teachable export unblocks it.
  - *Document/outline import* — a PDF/DOCX/pasted outline parsed into a draft module→lesson tree. Overlaps heavily with the already-specced AI Document Import Phase 2 (see memory `project_ai_document_import_phase2`); most likely the highest-leverage option, since it accepts what people actually have.
  - *YouTube playlist import* — **SHIPPED Aug 24, 2026** (see the checked line below). A playlist URL becomes one lesson per video with titles/descriptions pre-filled.
  - *Paste-an-outline quick path* — paste indented text, get a draft tree. Cheapest possible thing that beats hand-building; a good fallback if the richer options stall.

  **Constraints already known:** imports must land as drafts (matches Phase 1); file-backed lesson resources need a real upload (`fileKey`), so no import path can fake them from a URL; categories are match-only against the curated taxonomy (see the Aug 21 line below); plan limits slice the import via `canAddCourse`/`maxCourses`.

  **Success test:** a prospect with an existing course elsewhere gets a reviewable draft in AuthorLoft without retyping lesson titles. *(medium–large — scope it properly next session rather than picking a direction now)*
- [x] **YouTube playlist course import** — shipped Aug 24, 2026 as the first concrete piece of the ⭐ item above: paste a public playlist URL on `/admin/courses/import`, each video becomes a lesson (title + escaped description as content, watch URL in `videoUrl` — the `/learn` viewer already embeds those), lands as a single-module draft for the creator to reorganize. `src/lib/youtube-playlist.ts` (URL parsing, Data API v3 with pagination when `YOUTUBE_API_KEY` is set, playlist-RSS fallback capped by YouTube at 15 videos with a warning otherwise) + preview route `/api/admin/courses/import-youtube`; creation reuses `POST /api/admin/courses/import` unchanged, so plan limits/dedup/draft-only/side effects stay in one place. Set `YOUTUBE_API_KEY` (optional, in `required-env.json`) to lift the 15-video cap. *(done)*
- [ ] **Bulk course upload — onboarding entry point (Phase 2)** — surface the same importer inside the onboarding modal's course-setup phase, for course-creators migrating an existing curriculum during signup instead of building from scratch. Reuses `CourseImportWizard`/`/api/admin/courses/import` as-is. *(small — mostly wiring, once Phase 1 usage validates demand; note the ⭐ item above may change what gets surfaced here)*
