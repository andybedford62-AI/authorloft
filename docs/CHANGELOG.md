...
## September 21, 2026 — Alt text on marketing banners (Bing SEO flag)

Bing Webmaster flagged images without alt text. A crawl of all 127 sitemap pages on staging found 0 images missing the alt attribute but 28 with an empty alt="": the hero banner rendered by MarketingPageHeader on 26 pages (Features, Pricing, the calculator, Bookstore + 7 genre pages, all 12 solution pages, Blog, News, Resources, FAQ) plus one Bookstore spotlight cover. Empty alt is correct for decorative images, but Bing counts it as missing. MarketingPageHeader now takes an optional backgroundImageAlt (omitted = still decorative), every banner now passes a plain description of what the image shows, solution-page banners get theirs from a new backgroundImageAlt field in landing-page-data.tsx, and Bookstore spotlight covers use "Book cover by <author>". Not audited: author subdomain sites and the admin area.

## September 21, 2026 — Cost Savings Calculator (`/pricing/calculator`)

New public page that helps a prospective creator compare building and running their own site with AuthorLoft, in dollars and hours. Three plain-language steps: how many books/courses/music lists they make, which website features they want (basics / Standard / Premium groups), and what an hour of their time is worth. The results panel shows first-year cost of doing it yourself vs. the AuthorLoft plan that covers their picks, money and hours saved, time to launch, a "how we worked this out" walkthrough, and a "where the prices come from" list with vendor links. AuthorLoft plan prices and limits are read live from the `Plan` table (fallbacks in `savings-calculator-data.ts`), so a Super Admin price change flows through; the matching plan is derived from the features chosen plus the plan's book/course/music limits. DIY tool prices are current published prices checked Sept 21, 2026 (annual-billing monthly equivalent where offered); hours are estimates — refresh both, and bump `PRICES_CHECKED`, in `src/lib/savings-calculator-data.ts`. Entry points: gold callout at the top of `/pricing`, footer link, sitemap entry; the calculator page has a "Back to pricing" link and keeps Pricing highlighted in the nav. **Shareable preset links** for ads: `?preset=author|courses|music|all` prefills the counts (author = 5 books, courses = 3 courses, music = 4 music lists, all = 3/2/2), and `books=`, `courses=`, `music=`, `rate=`, `hours=` override individual values; the same four presets also appear as "Start with an example" chips. The preset keys are in live ads — don't rename them. The canonical URL ignores query strings, so UTM-tagged links don't split the page in search. Hero banner: `public/hero-pricing-calculator-real.webp`. Also: a "you vs. AuthorLoft" bar comparison in the results; a one-line DIY-vs-Free comparison under the plan cards on `/pricing` (numbers come from `basicsSnapshot()` so they can't drift from the calculator); links to the calculator from the `/compare` hub, `/features` CTA and every solution landing page; and consent-gated PostHog events (`calculator_started`, `calculator_preset_selected`, `calculator_cta_clicked` with matched plan, first-year savings and product counts) via new `captureEvent()` in `src/lib/posthog-client.ts`.

## September 13, 2026 — Fixed prod outage: bot file-probe scan exhausted the DB connection pool

A Vercel error-log alert flagged tens of thousands of `PrismaClientInitializationError`/`PrismaClientKnownRequestError` failures in a ~2-minute window, taking down `/[domain]/[pageSlug]` (author custom pages) and, via a shared connection pool, `/blog/[slug]` and `/news/[slug]` too. Root cause traced through Vercel runtime-error clusters and Supabase logs: a bot swept ~94 known "leaked AI-agent-secrets" filenames (`.aider.conf.yml`, `secrets.yaml`, `api_keys.json`, `mcp.json`, `.cursor/mcp.json`, `backup.sql`, etc.) against author domains. `[domain]/[pageSlug]` had no fast-path rejection for file-shaped slugs, so every guess triggered two real Prisma queries (`author.findFirst`, `authorPage.findFirst`) — once from `generateMetadata`, once from the page component, since `resolveAuthorAndPage()` wasn't deduped. ~50K queries in under 2 minutes exceeded Supavisor's (Supabase's pooler) 200-client connection cap — the DB's actual `max_connections` is only 60, Supavisor multiplexes on top of that. Fixed by rejecting any `pageSlug` not matching `[a-z0-9-]+` (the exact charset already enforced at page-creation time in `src/app/api/admin/pages/route.ts`) before any DB call — verified against the real scanned paths and against legitimate slugs. Verified fixed on staging and in production (`apbedford.authorloft.com/secrets.yaml` → clean 404, no DB hit).

## September 10, 2026 — Raised signed-URL TTL to 24h for customer downloads

Closing a gap from the egress work above: the signed-URL reuse fix caches the same URL until shortly before it expires, but the actual `expiresIn` values were untouched — still the original 3600s (1 hour) for orders/ARC/reader-magnet/course-resource downloads. Since the reuse-cache TTL is derived from `expiresIn`, a short TTL caps how long that cache stays warm regardless of the reuse logic — at 1 hour, even a popular file's CDN cache entry churns hourly. Added `SIGNED_URL_TTL_DAY` (86400s) to `supabase-storage.ts` and applied it to the five customer-facing download routes (`orders/download`, `reader-magnet/download`, `arc/download`, `courses/lesson-resource`, `courses/workbook-resource`). Left the two internal 300s ones (admin manuscript-conversion fetches) alone — single immediate use, no repeat-download benefit. Safe to do: the real access check (payment status, download count, link expiry) happens at the app layer on every request before a signed URL is even minted, and per Supabase's own docs, revoking a token doesn't purge its CDN cache entry anyway — deleting the object is the actual way to cut off access, so token TTL was never the security boundary.

Also worth noting from the same review: a live `curl -I` on a pre-fix object came back `Cache-Control: no-cache` (not the "1 hour default" Supabase's docs describe), confirming the upload-time cache-control fix from the first entry below is a bigger win than characterized there — but it's not retroactive; only objects uploaded after that fix shipped get the new header.

## September 10, 2026 — Fixed Supabase Storage cached-egress drivers

Two root causes were pushing Storage egress costs up (surfaced via the Supabase org usage dashboard, Free plan): (1) `uploadToSupabaseStorage` never sent a `cache-control` header, so every object fell back to Supabase's 1-hour default TTL; (2) `getSupabaseSignedUrl` minted a brand-new signed URL (unique `?token=`) on every download request (orders, reader-magnet, ARC, course resources), which is a guaranteed CDN cache miss every time per Supabase's own docs — signed-URL caching keys on the exact token. Fixed by (1) adding an explicit `cacheControl` param defaulting to Supabase's own 3600s, with `ONE_YEAR_CACHE` passed at every upload call site whose path already embeds a timestamp (covers, hero images, logos, profile photos, blog images, book previews, marketing/SEO/social images) and is therefore never overwritten in place; (2) caching signed URLs (Redis, in-memory fallback) keyed by bucket+path+expiry+filename and reusing them until shortly before token expiry, so repeat downloads of the same file become cache hits instead of fresh origin fetches. Extracted the Redis client bootstrap (`getRedisClient`) out of `rate-limit.ts` into a new shared `src/lib/redis-client.ts` so both modules reuse the same connect/circuit-breaker logic rather than duplicating it. A follow-up pass confirmed the remaining upload routes (course files, audio, flip-book covers, specials, book files) also use `Date.now()`-random unique filenames, so those got `ONE_YEAR_CACHE` too. Supabase's Smart CDN (query-string-agnostic caching, higher hit rates) only auto-enables on Pro plan and above; this project is currently on Free.

## September 10, 2026 — Compress/resize images on upload to reduce Storage egress further

Caching (above) cuts how often a file gets re-transferred; this cuts how many bytes each transfer costs. No image processing existed on upload at all — whatever an author's browser/camera produced (easily 3-6MB per cover) was stored and served byte-for-byte forever, and several admin/marketing surfaces (bookstore quick-view, resources/guides index, admin list pages) render plain `<img>` instead of `next/image`, so they get zero automatic resizing either. Added `src/lib/image-processing.ts` (`optimizeImageForWeb`, using `sharp`, now a direct dependency): caps the longest side to a per-surface max (2000-2400px for covers/heroes/blog images, 1200px for profile photos, 1000px for logos), auto-orients from EXIF then strips it, and re-encodes as WebP (lossless for logos, quality 82 elsewhere) — a synthetic 3000x2000 test image dropped from 5.1MB to 1.08MB (79%) even with adversarial noise; real photos compress further. Falls back to the original bytes if re-encoding ever comes out larger or throws. Wired into every image upload route (covers, hero images, logos, profile photos, blog images, book previews, specials, flip-book covers, marketing/blog/SEO images). Two routes keep `convertFormat: false` (resize/recompress only, original format preserved) because WebP isn't safe there: SEO OG images (social-media link-preview crawlers don't reliably render WebP) and social-post images (forwarded to third-party posting APIs that expect JPEG/PNG). GIF and SVG are always left untouched (animation risk / vector, respectively). Swapping the remaining raw `<img>` surfaces to `next/image` is a separate, smaller follow-up.

## September 10, 2026 — Swapped public-facing `<img>` tags to next/image

Third and final pass on the Storage egress work above: `next/image` resizes/re-encodes on the fly and caches the result at Vercel's edge, so a page using it never re-pulls the full-size original per view the way a plain `<img>` does. Found 44 files still using `<img>` — almost all deliberately, marked with `eslint-disable-next-line @next/next/no-img-element`. Split into two real categories: file-picker previews built on `URL.createObjectURL()` (confirmed in `onboarding-guided-modal.tsx` and presumably others) which `next/image` genuinely can't optimize (it can't proxy a `blob:` URL), and everything else. Converted the 21 files that are both public-facing (real traffic) and not blob-preview cases: the 7 auth pages (shared static logo), `news/[slug]`, `blog/[slug]`, `guides/[slug]` (+ its related-posts thumbnail), `resources/[slug]`, `resources` index, `guides-index-client`, `bookstore-quick-view`, `bookstore` page (spotlight avatar + sample covers), `marketing-page-header` (shared banner used across Bookstore/Blog/News/Resources/etc.), `rebrand-hero` (homepage — 3 static full-image cards), and the author-site `courses/[slug]`, `courses` index, `bundles/[slug]`, and `reader-magnet-modal`. Used `fill` inside a `relative`-positioned, explicitly-sized wrapper for variable-aspect covers (adding the wrapper/`relative` where one didn't already exist), and explicit `width`/`height` for fixed-size assets (the auth logo, avatars, the reader-magnet cover). Left the other 23 files (admin/super-admin panels, blob-preview components) on `<img>` — low/no public traffic there, so little egress payoff, and several would need real behavior changes (`unoptimized` mode, or restructuring around blob previews) to convert safely.

## September 10, 2026 — Blog/guides cover images: fill the box instead of letterboxing; embedded body images stretch to column width

Two small visual fixes surfaced right after the `next/image` swap above, both pre-existing behavior that just got more visible. (1) `blog/[slug]` and `guides/[slug]` cover images used `object-contain`, which preserves the full image but leaves a background gap on whichever axis doesn't match the box's aspect ratio — switched both to `object-cover` (crop-to-fill) to match `news/[slug]`, which already did this. Left `resources/[slug]`, the `resources` index, and `bookstore-quick-view` on `object-contain` — those are document/book covers where cropping could cut off cover text, a different tradeoff than a blog hero photo. (2) One published post (`author-needs-a-website-in-2026`) has a body image embedded via the rich-text editor that's only 162×152px natively — `.rich-content img` / `.tiptap-editor img.rte-image` in `globals.css` only had `max-width:100%`, which caps size but never stretches a small image up, so it rendered tiny and left-aligned. Added `width: 100%` since the editor's `ImageExtension` is `inline: false` (block-only inserts, never small inline icons), so every embedded image should span the column regardless of its native size — confirmed via SQL that this is the only published post/guide with an inline image anywhere on the site, so no other content is affected. That one image will still look soft since it's a ~4-5x upscale of a genuinely low-res source; the layout is fixed but the author should re-upload it at a higher resolution to look sharp.
