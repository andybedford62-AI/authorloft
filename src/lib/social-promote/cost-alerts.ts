...
export async function checkCostCeilingsAfterGen(opts: {
  ...
  try {
    ...
    const prevCents       = (todayCents - opts.thisGenCostMicroCents / 10_000);
    const warnCents       = settings.costWarnCentsPerDay;
    const disableCents    = settings.costDisableCentsPerDay;

    const justCrossedWarn    = prevCents < warnCents    && todayCents >= warnCents    && warnCents > 0;
    const justCrossedDisable = prevCents < disableCents && todayCents >= disableCents && disableCents > 0;
    ...
    if (justCrossedDisable) {
      await prisma.socialPromoteSettings.update({ where: { id: "main" }, data: { enabled: false } });
      await sendDisableEmail(recipients, todayCents, disableCents);
    } else if (justCrossedWarn) {
      await sendWarnEmail(recipients, todayCents, warnCents, disableCents);
    }
  } catch (err) {
...
async function sendWarnEmail(recipients: string[], todayCents: number, warnCents: number, disableCents: number) {
  ...
  const html = wrapHtml(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;margin:0 0 14px;font-size:14px;">
      <tr><td style="padding:6px 12px;background:#fef3c7;border-radius:4px 0 0 4px;">Spend so far today</td><td style="padding:6px 12px;background:#fef3c7;border-radius:0 4px 4px 0;font-weight:700;">${fmtUsd(todayCents)}</td></tr>
      <tr><td style="padding:6px 12px;">Warn ceiling</td><td style="padding:6px 12px;">${fmtUsd(warnCents)}</td></tr>
      <tr><td style="padding:6px 12px;">Auto-disable ceiling</td><td style="padding:6px 12px;">${fmtUsd(disableCents)}</td></tr>
    </table>
  ...
  await sendMail({ to, subject: `[Social Promote] Cost warning — ${fmtUsd(todayCents)} today`, html, text: `Social Promote spend today: ${fmtUsd(todayCents)} (warn ceiling: ${fmtUsd(warnCents)}, auto-disable: ${fmtUsd(disableCents)})` });
}

async function sendDisableEmail(recipients: string[], todayCents: number, disableCents: number) {
  const to = recipients.join(", ");
  const html = wrapHtml(
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;margin:0 0 14px;font-size:14px;">
      <tr><td style="padding:6px 12px;background:#fee2e2;border-radius:4px 0 0 4px;">Spend today</td><td style="padding:6px 12px;background:#fee2e2;border-radius:0 4px 4px 0;font-weight:700;">${fmtUsd(todayCents)}</td></tr>
      <tr><td style="padding:6px 12px;">Auto-disable ceiling</td><td style="padding:6px 12px;">${fmtUsd(disableCents)}</td></tr>
    </table>
  ...
  await sendMail({ to, subject: `[Social Promote] AUTO-DISABLED — ${fmtUsd(todayCents)} reached`, html, text: `Social Promote auto-disabled. Today's spend ${fmtUsd(todayCents)} reached the ${fmtUsd(disableCents)} ceiling. Re-enable at /super-admin/social-promote/settings after investigating.` });
}
