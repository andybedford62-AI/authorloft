...
export default async function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: VAULT.bg, color: VAULT.ink, fontFamily: VAULT.fontBody, fontSize: '1rem', lineHeight: 1.6, WebkitFontSmoothing: 'antialiased' }}>
      ...
      <section style={{ background: VAULT.surf, borderTop: `1px solid ${VAULT.line}`, borderBottom: `1px solid ${VAULT.line}`, padding: '88px 28px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <p style={{ color: VAULT.mute, fontSize: '1.0625rem', marginBottom: 48 }}>No credit card. Upgrade when you want direct sales or a custom domain.</p>
          {foundingOffer && (
            <div style={{
...
function ProblemStrip() {
  ...
  return (
    <div style={{ background: VAULT.bgDeep, borderTop: `1px solid ${VAULT.line}`, borderBottom: `1px solid ${VAULT.line}`, padding: '52px 0' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 28px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 0 }}>
          {items.map((item, i) => (
            <div key={i} style={{ textAlign: 'left', padding: '24px 32px', borderRight: i < items.length - 1 ? `1px solid ${VAULT.line}` : 'none' }}>
              <span style={{ fontSize: '1.75rem', marginBottom: 14, display: 'block' }}>{item.icon}</span>
              <h3 style={{ fontFamily: VAULT.fontDisplay, fontStyle: 'italic', fontSize: '1.25rem', fontWeight: 600, marginBottom: 8, color: VAULT.ink, lineHeight: 1.12 }}>{item.title}</h3>
              <p style={{ fontSize: '0.9rem', color: VAULT.mute, lineHeight: 1.65 }}>{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
      <style>{`@media (max-width: 820px) { .problem-strip-grid > div { border-right: none !important; border-bottom: 1px solid ${VAULT.line}; } .problem-strip-grid > div:last-child { border-bottom: none; } }`}</style>
    </div>
  );
}
...
function PillarsSection() {
  ...
  return (
    <section style={{ padding: '88px 0', background: VAULT.bg }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 28px' }}>
        <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: VAULT.gold, marginBottom: 14 }}>The solution</p>
        <h2 style={{ fontFamily: VAULT.fontDisplay, fontSize: 'clamp(2.1rem, 4vw, 3.4rem)', fontWeight: 600, lineHeight: 1.12, fontStyle: 'italic', color: VAULT.ink, marginBottom: 16, letterSpacing: '-0.01em' }}>
          One platform.<br />Everything you own.
        </h2>
        <p style={{ fontSize: '1.0625rem', color: VAULT.mute, maxWidth: 560, margin: '0 0 52px', lineHeight: 1.72 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 2, background: VAULT.line, borderRadius: 14, overflow: 'hidden', textAlign: 'left' }}>
          {pillars.map((p) => (
            <div key={p.num} style={{ background: VAULT.surf, padding: '40px 32px' }}>
              <div style={{ fontFamily: VAULT.fontDisplay, fontSize: '3.5rem', fontWeight: 700, color: 'rgba(214,169,74,0.18)', lineHeight: 1, marginBottom: 18 }}>{p.num}</div>
              <h3 style={{ fontFamily: VAULT.fontDisplay, fontStyle: 'italic', fontSize: '1.35rem', fontWeight: 600, lineHeight: 1.12, color: VAULT.ink, marginBottom: 10 }}>{p.title}</h3>
              <p style={{ color: VAULT.mute, fontSize: '0.9375rem', lineHeight: 1.65 }}>{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
function ComparisonSection() {
  ...
  return (
    <section style={{ background: VAULT.bg, borderTop: `1px solid ${VAULT.line}`, borderBottom: `1px solid ${VAULT.line}`, padding: '88px 0' }}>
      <style>{`
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 28px' }}>
        <div style={{ marginBottom: 44, maxWidth: 640 }}>
          <p style={{ fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: VAULT.gold, margin: '0 0 14px' }}>The difference</p>
          <h2 style={{ fontFamily: SERIF, fontSize: 'clamp(1.85rem, 3vw, 2.6rem)', fontWeight: 600, lineHeight: 1.12, fontStyle: 'italic', color: VAULT.ink, letterSpacing: '-0.01em', margin: 0 }}>
            What changes when you own it.
          </h2>
        </div>
        <div className="rdh-cmp-table">
          {/* Column headers */}
          <div className="rdh-cmp-old-head" style={{ padding: '18px 32px', fontSize: '0.8125rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', background: VAULT.bg, textAlign: 'left', color: VAULT.mute, borderBottom: `2px solid ${VAULT.line}` }}>The old way</div>
          ...
          {rows.map((row, i) => (
            <div key={row.label} style={{ display: 'contents' }}>
              ...
              <div className="rdh-cmp-old-cell" style={{ background: VAULT.bg, padding: '32px 36px', borderBottom: i < rows.length - 1 ? `1px solid ${VAULT.line}` : 'none' }}>
                ...
                <div style={{ fontFamily: SERIF, fontSize: '1.35rem', lineHeight: 1.4, color: VAULT.mute, textDecoration: 'line-through', textDecorationColor: 'rgba(255,80,80,0.5)', fontStyle: 'italic' }}>{row.old}</div>
              </div>
              {/* New way cell */}
              <div style={{ background: VAULT.surf2, padding: '32px 36px', borderLeft: `2px solid ${VAULT.line}`, borderBottom: i < rows.length - 1 ? `1px solid ${VAULT.line}` : 'none' }}>
                <div style={{ fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: VAULT.mute, marginBottom: 10, fontWeight: 700 }}>{row.label}</div>
                <div style={{ fontFamily: SERIF, fontSize: '1.5rem', lineHeight: 1.4, color: VAULT.ink }}>
...
function NewsletterMidSection() {
  return (
    <section style={{ background: '#f5f0e8', borderTop: '1px solid #d8ceb8', borderBottom: '1px solid #d8ceb8', padding: '88px 0' }}>
...
function FooterCTA() {
  return (
    <section style={{ padding: '112px 0', position: 'relative', overflow: 'hidden', background: VAULT.bg }}>
      {/* Glow */}
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 28px', position: 'relative' }}>
        ...
        <p style={{ fontSize: '1.125rem', color: VAULT.mute, marginBottom: 44, maxWidth: 460, lineHeight: 1.7 }}>
