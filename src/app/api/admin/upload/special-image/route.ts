...
export async function POST(req: NextRequest) {
  ...
  if (SUPABASE_CONFIGURED) {
    ...
    try {
      ...
      console.error("[upload/special-image] Supabase error:", detail);

      if (detail.includes("404") || detail.includes("Bucket not found") || detail.includes("NoSuchBucket")) {
        return NextResponse.json(
          { error: 'Supabase bucket "specials" does not exist. Please create it in your Supabase dashboard (Storage → New bucket → name: specials, Public: on).' },
          { status: 500 }
        );
      }
      if (detail.includes("403") || detail.includes("Unauthorized") || detail.includes("policy")) {
        return NextResponse.json(
          { error: 'Supabase Storage permission denied. Make sure the "specials" bucket is set to Public and your service role key has storage access.' },
          { status: 500 }
        );
      }
    ...
    publicUrl = `/uploads/specials/${filename}`;
    fileKey   = null;
  }
