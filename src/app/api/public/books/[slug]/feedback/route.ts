...
export async function POST(
  ...
  try {
    ...
    // ── Rate limit: max 1 submission per email per book per 24h ──────────────
    const oneDayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);
    const recentCount = await prisma.bookFeedback.count({
      where: {
        bookId: book.id,
    ...
    if (recentCount >= 1) {
      return NextResponse.json(
        { error: "You've already submitted feedback for this book. Please wait 24 hours." },
        { status: 429 },
      );
    }

    // ── Create feedback ───────────────────────────────────────────────────────
    await prisma.bookFeedback.create({
      data: {
        bookId: book.id,
    ...
    if (author.contactEmail) {
      ...
      const textBody = [
        `New reader feedback for "${book.title}"`,
        ``,
      ...
      const htmlBody = `
        <div style="font-family: sans-serif; max-width: 580px; margin: 0 auto; color: #1a1a1a;">
          <div style="background: #2563EB; padding: 20px 24px; border-radius: 8px 8px 0 0;">
            ...
            <p style="margin:4px 0 0; color: rgba(255,255,255,0.75); font-size: 14px;">For &ldquo;${esc(book.title)}&rdquo;</p>
          </div>
      ...
      await sendMail({
        ...
        subject: `New reader feedback on "${book.title}"`,
        text: textBody,
        html: htmlBody,
      });
    }
    ...
    console.error("[book-feedback] Error:", err);
