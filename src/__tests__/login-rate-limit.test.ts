...
describe("checkLoginRateLimit", () => {
  ...
  it("allows the first 5 attempts for an email", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      expect(await checkLoginRateLimit("user@example.com")).toBe(true);
    }
  });
  ...
  it("treats emails case-insensitively", async () => {
    const { checkLoginRateLimit } = await import("@/lib/login-rate-limit");
    for (let i = 0; i < 5; i++) await checkLoginRateLimit("User@Example.COM");
    expect(await checkLoginRateLimit("user@example.com")).toBe(false);
  });
