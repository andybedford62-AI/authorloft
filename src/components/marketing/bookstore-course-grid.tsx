  ...
  return (
    <div>
      ...
      {allCategories.length > 0 && (
        <div className="bg-vault-surf rounded-xl border border-vault-ink/12 p-3 mb-5">
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {hasActiveFilters && (
              <button
                type="button"
                onClick={resetAll}
                className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-vault-gold hover:text-vault-ink transition-colors"
              >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-vault-surf rounded-2xl border border-vault-ink/12">
          <GraduationCap className="h-10 w-10 text-vault-mute mx-auto mb-3" />
          <p className="text-vault-mute mb-4">No courses match your selected categories.</p>
          <button
            type="button"
            onClick={resetAll}
